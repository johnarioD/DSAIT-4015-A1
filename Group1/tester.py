...
class ModelTester:
    ...
    def run_partitioning_tests(self, df: pd.DataFrame, features: List[str], threshold: float = 0.15):
        ...
        for feature in features:
            """
            TODO: chech the nunique logic for "contacten_onderwerp__werk_intake" features
            """
            if feature not in df.columns:
                continue

            unique_vals = temp_df[feature].nunique()
            ...
            if unique_vals <= 5:
                group_means = temp_df.groupby(feature)['_risk'].mean()
                partitions = group_means.values
                msg_details = f"Categorical/Small split (Unique={unique_vals})"
            elif data_range <= 50:
                median = temp_df[feature].median()
                g1 = temp_df[temp_df[feature] <= median]['_risk'].mean()
                g2 = temp_df[temp_df[feature] > median]['_risk'].mean()
                partitions = [g1, g2]
                msg_details = f"Binary split at median {median:.1f} (Range<=50)"

            else:
                try:
                    temp_df['bin'] = pd.qcut(temp_df[feature], q=3, labels=False, duplicates='drop')
                    group_means = temp_df.groupby('bin')['_risk'].mean()
                    partitions = group_means.values
                    msg_details = f"Ternary split (Low/Mid/High) based on quantiles (Range>50)"
                except Exception as e:
                    median = temp_df[feature].median()
                    g1 = temp_df[temp_df[feature] <= median]['_risk'].mean()
                    g2 = temp_df[temp_df[feature] > median]['_risk'].mean()
                    partitions = [g1, g2]
                    msg_details = f"Fallback binary split (qcut failed: {str(e)})"


            partitions = [p for p in partitions if not np.isnan(p)]
            
            if len(partitions) < 2:
                diff = 0.0
                passed = True
                msg = "Skipped: Not enough data partitions"
            else:
                diff = max(partitions) - min(partitions)
                passed = diff < threshold
                msg = f"Max diff: {diff:.4f} | {msg_details}"
...
if __name__ == "__main__":
    MODELS_TO_TEST = ["model/good_model.onnx", "model/bad_model.onnx"]
    DATA_PATH = "data/synth_data_for_training.csv"
    TARGET_COLUMNS_TO_DROP = ['checked'] 
    ...
    for model_path in MODELS_TO_TEST:
        ...
        # TODO: # refine thresholds function
        tester.run_partitioning_tests(df, features=all_features_to_test, threshold=0.15)
        tester.run_metamorphic_tests(df, features=all_features_to_test, threshold=0.15)
